// Xiaoxiao: edited variables.C, not for public use (with TMVAGui.C for example)!
...
void variables( TString fin = "TMVA.root", TString outBin = "", TString dirName = "InputVariables_Id", TString title = "TMVA Input Variables",
                Bool_t isRegression = kFALSE, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)next())) {
      ...
      if (countPad%noPadPerCanv==0) {
    	 ...
    	 TString fname = Form( "plots/%s_%s_c%i", outBin.Data(), outfname.Data(), countCanvas );
         TMVAGlob::plot_logo();
